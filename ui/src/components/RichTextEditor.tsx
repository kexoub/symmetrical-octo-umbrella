...
export const RichTextEditor = ({ value, onChange }: { value: string, onChange: (html: string) => void }) => {
    ...
    const handleFileUpload = useCallback(async (file: File, type: 'image' | 'attachment') => {
        ...
        try {
            ...
            toast({ title: "上传成功", description: `${type === 'image' ? '图片' : '附件'}已插入到编辑器中` });
        } catch (error: any) {
        ...
        <div className="border border-gray-300 rounded-sm bg-white">
            <div className="flex items-center justify-between p-1 border-b border-gray-200 bg-gray-50 flex-wrap">
                <div className="flex items-center flex-wrap">
                    <Select onValueChange={(val) => val === 'p' ? editor.chain().focus().setParagraph().run() : editor.chain().focus().toggleHeading({ level: parseInt(val) as any }).run()}>
                        <SelectTrigger className="w-24 h-6 text-xs mr-1">
                            <SelectValue placeholder="大小" />
                        </SelectTrigger>
                        <SelectContent>
                            <SelectItem value="p">段落</SelectItem>
                            {[1, 2, 3, 4, 5, 6].map(h => <SelectItem key={h} value={`${h}`}>标题 {h}</SelectItem>)}
                        </SelectContent>
                    </Select>
                    ...
                    <div className="h-4 border-l mx-1"></div>
                    <ToolbarButton onClick={() => { toast({ title: "提示", description: "表情功能即将推出" }); }} tip="表情"><Smile size={16} /></ToolbarButton>
                    <ToolbarButton onClick={() => imageInputRef.current?.click()} tip="上传图片"><ImageIcon size={16} /></ToolbarButton>
                    <ToolbarButton onClick={() => attachmentInputRef.current?.click()} tip="上传附件"><Paperclip size={16} /></ToolbarButton>
                    <ToolbarButton onClick={handleLinkInsert} tip="插入链接"><LinkIcon size={16} /></ToolbarButton>
                </div>
            </div>
